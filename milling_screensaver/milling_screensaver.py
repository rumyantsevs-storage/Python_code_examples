...
while running:
    ...
    from_left_to_right = random.choice((True, False))
    # Tool Y position
    tool_point_y = random.uniform(200.0, RESOLUTION[1] - 200.0)
    # Plate number in tool
    nplates = random.choice((2, 3, 4, 6, 8, 22))
    # Tool axis runout
    eccentricity = random.uniform(1.0, 5.0)
    # Tool CW revolutions
    clockwise_rotation = True
    # Tool revolution coefficient
    angle_coeff = random.uniform(1.0, 4.0)
    # Tool speed coefficient
    speed_coeff = random.uniform(1.0, 3.0)
    ...
    while stage:
        ...
        mill.center_x = axis.point_x
        mill.center_y = axis.point_y
        mill.apply_params()
        mill.draw_plates()
